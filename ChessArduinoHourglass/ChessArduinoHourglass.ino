...
void loop() {
  ...
  if ( CurrentPlayerTimeIndex == 0 ) {                                   // In order for both players to know the system is working, the first SandGrain is forced to drop
    delay (1000);
    hourglassanimation.RemoveSandGrain(1, TopMatrixAddress);             // These two lines describe the drop of one SandGrain
  ...
  while (TimeHasBeenSelected == 0) {
    ...
    if (FindIfHourglassWasFlipped() == 1) {                     //Check if the pushbutton is pressed.if it is, the TiltSwitchState is HIGH:
      ...
      while (FindIfHourglassWasFlipped() == 1) {                //infinite loop while the button is pressed
        ...
        delay(90);                                              // delay introduced to avoid detecting rebounds of the marble in the TiltSwitch as increments of the counter
       }
...
int TimeIndex() {
  if (ItsP1TurnToPlay == digitalRead(TiltSwitchPin)) {                             // Depending on whose turn it is, ElapsedTime is updated
    ...
    return (TimeElapsedP1 * 64) / TotalTimeForEachPlayer ;                         // Time index is calculated as a fraction of the 64 leds of the matrix.
  } else {
    TimeElapsedP2 = (millis() - TimeAtStart) / 1000 - TimeElapsedP1 ;
    return (TimeElapsedP2 * 64) / TotalTimeForEachPlayer ;
  }
}
